...
if($linhas == 0){
	...
	$res_d   = $pdo->query("SELECT * FROM advogados WHERE cpf = '$cpf_adv' ");
	$dados_d = $res_d->fetchAll(PDO::FETCH_ASSOC); 
	$email_adv	 = $dados_p[0]['email'];
	...
	$url_painel_adv  = $url_sistema.'advogado';
	$to       = $email_adv;
	$subject  = "Marcação de Audiência dia $data2";
