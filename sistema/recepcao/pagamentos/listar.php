...
if(($dataInicial == '' or $dataFinal == '') and $txtbuscar == '%%' ){
    $res = $pdo->query("SELECT * from pagamentos order by id asc");
} elseif($dataInicial == '' or $dataFinal == '') {
	$res = $pdo->query("SELECT * from pagamentos where funcionario LIKE '$txtbuscar' or nome_funcionario LIKE '$txtbuscar' order by id asc");
}else{
...
<tfoot>
<tr><td colspan="6">
<div class="float-right totalpago">Total Pago: R$ '. str_replace('.', ',', @$total).'</div>
</td></tr>
</tfoot>
