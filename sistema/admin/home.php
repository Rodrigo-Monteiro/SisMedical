...
$res_c = $pdo->query("SELECT * from audiencias where data_audiencia = curDate()");
		...
		<div class="col-sm-12 col-lg-3 col-md-6 col-sm-6 mb-4">
							...
							<div class="row">
								...
								<div class="col-7 col-md-8">
									<div class="numbers">
										...
										<p class="subtitulo-card"><?php echo $total ?><p>
										</div>
									</div>
								</div>
							</div>
			...
			<div class="mt-4">
				...
				<div class="row">
					...
					<?php 
					$res = $pdo->query("SELECT * from audiencias where data_audiencia = curDate() and hora_audiencia >= curTime() order by hora_audiencia asc limit 8");
					$dados = $res->fetchAll(PDO::FETCH_ASSOC);
					for ($i=0; $i < count($dados); $i++) { 
						foreach ($dados[$i] as $key => $value) {
						}
						...
						$hora = $dados[$i]['hora_audiencia'];
						$data = $dados[$i]['data_audiencia'];
