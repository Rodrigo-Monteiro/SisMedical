...
echo '
<table class="table table-responsive-sm mt-3" tabelas>
	...
	if($txtbuscar == ''){
		$res = $pdo->query("SELECT * from clientes order by nome asc LIMIT $limite, $itens_por_pagina");
	}else{
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
		$res = $pdo->query("SELECT * from clientes where nome LIKE '$txtbuscar' or cpf LIKE '$txtbuscar' order by nome asc");
